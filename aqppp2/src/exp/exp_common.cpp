...
namespace expDemo {
	...
	std::pair<double, double> ReadBLBSamples(SQLHANDLE& sqlconnectionhandle, aqppp::Settings& o_PAR, int sample_num, std::vector <std::vector<std::vector<double>>>& blb_samples)
	{
		//blb_sample = std::vector<std::vector<double>>();
		//o_small_sample = std::vector<std::vector<double>>();
		std::vector<std::vector<double>> blb_sample;
		std::string name = "blb_subsample";
		std::string subsample_name;
		std::vector<double> read_BLB_sample_times = std::vector<double>();
		
		for (int i = 0; i < sample_num; i++)
		{
			subsample_name = name + std::to_string(i+1);
			double blb_sample_time = aqppp::SqlInterface::ReadBLBDb(sqlconnectionhandle, blb_sample, o_PAR.DB_NAME, subsample_name, o_PAR.AGGREGATE_NAME, o_PAR.CONDITION_NAMES);
			read_BLB_sample_times.push_back(blb_sample_time);
			std::cout << "Read done for BLB_subsample # " << i << std::endl;
			blb_samples.push_back(blb_sample);
		}
		double time_read_BLB_sample = aqppp::Tool::get_percentile(read_BLB_sample_times, 0.5);
		//o_PAR.SAMPLE_ROW_NUM = blb_samples[0].size();
		std::cout << "o_sample" << blb_samples[0].size();
		return{ time_read_BLB_sample,0.0 };
	}
	...
	double QueryRealValue(const std::vector<aqppp::Condition>& cur_q, std::string table_name, SQLHANDLE &sqlconnectionhandle, const aqppp::Settings PAR, std::string agg_action, std::vector<std::unordered_map<int, std::string>> &distinct_itos, bool CLEANCACHE) {
		...
		for (int ci = 0; ci < DIM; ci++)
		{
			...
			ub_str = "'" + ub_str + "'";
			bk += PAR.CONDITION_NAMES[ci] + ">=" + lb_str + " OR " + PAR.CONDITION_NAMES[ci] + "<=" + ub_str;
		}
