...
namespace Exp {
	...
	double Exp_groupby::gb_create_db_sample(SQLHANDLE &sqlconnectionhandle, int seed, string db_name, string table_name, double sample_rate, string sample_name,const vector<string> &GROUP_CONDITIONS, GpType4Str &o_group_rates)
	{
		...
		for (auto gp : o_group_rates)
		{
			...
			{
				first_ele = false;
				create_sample += "SELECT *  INTO " + sample_full_name + " FROM " + table_full_name + " WHERE " + SqlInterface::ComputeRandStr(seed,gp.second) + st+";";
			}
			else
			{
				create_sample += "INSERT INTO " + sample_full_name + " SELECT * FROM " + table_full_name + " WHERE " + SqlInterface::ComputeRandStr(seed,gp.second) + st + ";";
			}
		}

		cout << drop_sample << endl;
		cout << create_sample << endl;
		SqlInterface::SqlQuery(drop_sample, sqlstatementhandle);
		double t1 = clock();
		SqlInterface::SqlQuery(create_sample, sqlstatementhandle);
	...
	void Exp_groupby::get_groups(const vector<vector<double>>& sample, const vector<vector<double>>& sub_sample, const GpType4Int &gp_rates4sample, const GpType4Int &gp_rates4sub_sample, const int GB_LEN, unordered_map<vector<int>, Group, VectorHash> &o_groups)
	{
		o_groups = unordered_map<vector<int>, Group, VectorHash>();
		//init group by
	...
	int Exp_groupby::exp(SQLHANDLE& sqlconnectionhandle)
	{
		...
		unordered_map<vector<int>, Group, VectorHash> groups;
		...
		Tool::TransSample(sample, CAsample);
		...
		{
			double t1 = clock();
			AssignBudgetForDimensions(PAR, false).AssignBudget(CAsample, mtl_nums);
			...
			HillClimbing(PAR.SAMPLE_ROW_NUM, PAR.SAMPLE_RATE, PAR.CI_INDEX, PAR.NF_MAX_ITER, PAR.INIT_DISTINCT_EVEN).ChoosePoints(CAsample, mtl_nums, NF_mtl_points, max_errs, iter_nums);
			...
			double time_NF_mtl_res = 0;
			if (exp_par.isMTL) time_NF_mtl_res=Precompute(PAR.DB_NAME, PAR.TABLE_NAME, PAR.AGGREGATE_NAME, PAR.CONDITION_NAMES).GetPrefixSumCube(NF_mtl_points, sqlconnectionhandle, NF_mtl_res, "sum", distinct_stoi);
		...
		*/
		...
		{
			vector<vector<CA>> tp_casample;
			Tool::TransSample(gpit.second.sample, tp_casample);
			vector<vector<Condition>> tp_user_queries = vector<vector<Condition>>();
			Tool::GenUserQuires(gpit.second.sample, tp_casample, PAR.RAND_SEED, query_num, { exp_par.MIN_QUERY_SELECTIVELY,exp_par.MAX_QUERY_SELECTIVELY }, tp_user_queries,groups);
		...
		for (int query_id = 0; query_id < user_queries.size(); query_id++)
		{
			...
			for (auto& gpit : groups)
			{
				...
				*/
				...
				double t1 = clock();
				pair<double, double> spl_sumci = Sampling(g.sample_rate, PAR.CI_INDEX).SamplingForSumQuery(g.sample, cur_q);
				double time_spl = (clock() - t1) / CLOCKS_PER_SEC;

				if (DoubleEqual(spl_sumci.first, 0.0)) continue;   //if not found in sample, then skip this query. 
				pair<double, double> aqpp_sumci;
				double time_aqpp = 0;
				if (exp_par.EXP_NO_UNIFORM == true)
				{
					double t1 = clock();
					aqpp_sumci = Aqpp(g.sample[0].size(), g.sample_rate, PAR.CI_INDEX).AqppSumQuery(query_id, info_file, g.sample, g.small_sample, NF_mtl_points, NF_mtl_res, cur_q);
					time_aqpp = ((double)clock() - (double)t1) / CLOCKS_PER_SEC;
				}

				double t2 = clock();

				double real_value = expDemo::QueryRealValue(cur_q, PAR.TABLE_NAME, sqlconnectionhandle, PAR, "sum", distinct_itos);
...
bool Exp::Exp_groupby::operator==(const Exp_groupby& other) const
{
	return false;
}
